...
class MirrorSimulation:
    def __init__(self, mirror_orbit_params):
        self.dt = 0.01
        self.t = 0
        self.G = 5000

        # 태양과 화성 관련 파라미터 (실제 타원 궤도)
        self.sun_mass = 1e10
        self.mars_mass = 1e8

        # 화성 궤도 케플러 요소 (실제 타원 궤도)
        ...
        self.mirror_orbit = KeplerOrbit(
            ...
            a=97.4492,
            e=0.2898,
            i=math.radians(5.4293),
            Omega=math.radians(88.5311),
            omega=math.radians(26.0991),
            M0=math.radians(360.0)
        )
    ...
    def compute_reflected_energy(self):
        ...
        reflected_energy = solar_constant_at_1AU * mirror_area * incidence_angle * attenuation
    ...
    def draw(self):
        ...
        self.ax_orbit.set_xlabel('X (space units)')
        self.ax_orbit.set_ylabel('Y (space units)')
        self.ax_orbit.set_zlabel('Z (space units)')

        self.ax_orbit.set_title('Sun, Mars and Mirror Orbit')
        self.ax_orbit.legend(loc='upper right')

        # 반사 에너지 그래프
        self.ax_energy.plot(self.energy_trace, color='orange')
        self.ax_energy.set_title('Reflected Solar Energy')
...
# 주어진 우주 거울 파라미터 적용 (이미 클래스 내부에 반영됨)
mirror_orbit_params = {
    'a': 97.4492,
    'e': 0.2898,
    'i': math.radians(5.4293),
...
sim = MirrorSimulation(mirror_orbit_params)
sim.run()
